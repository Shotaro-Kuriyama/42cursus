...
フロントエンド（tui（文字でguiを表現している）があるということ）である。
...
ここでいう MAC は、ネットワークの MACアドレス じゃなくて、
Mandatory Access Control（強制アクセス制御） の略です。
MAC（Mandatory Access Control）とは
OSが「このプロセス（アプリ）は、ここまでしか触っていいよ」と ルールを強制する 方式のアクセス制御です。
・ルールは 管理者が定義（またはシステムが持つポリシー）
・ユーザーやプログラムが勝手に変更できない
・だから、もしアプリが乗っ取られても「許可されてない範囲」には手が出せない

AppArmorはまさにこれで、プログラム単位で “触っていいファイル・機能・ネットワーク” をプロファイルで縛る 仕組みです。
