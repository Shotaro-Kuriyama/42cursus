...
✅ その場で「証拠」を見せるコマンド（本番用）
sudo ufw status verbose
sudo ufw status numbered


もし「4242だけ許可した証拠は？」と言われたら：
sudo ufw allow 4242/tcp
sudo ufw status
...
ポートとroot禁止の設定（設定ファイル上の証拠）
sudo grep -E "Port|PermitRootLogin" /etc/ssh/sshd_config

実際に4242で待ち受けしてる証拠（最強）
sudo ss -tulpn | grep sshd

firewall的に4242だけ開いてる証拠
sudo ufw status verbose
...
（あなたの環境で sudo_config がそこに出てれば「ログ保存してます」が一撃で証明できます）
...
✅ 証拠を見せるコマンド（ここも一緒に出すと強い）

評価者が「じゃあ設定どこ？」と言った瞬間にこれです。

期限系（30日/2日/7日）
sudo chage -l yourlogin
sudo grep -E "PASS_MAX_DAYS|PASS_MIN_DAYS|PASS_WARN_AGE" /etc/login.defs

強い文字条件（大文字小文字数字、連続禁止など）
sudo cat /etc/pam.d/common-password
...
/（スラッシュ）で〇〇分置き、なしだと毎時10分に出力（10;10, 11;10, 12;10. ...）
...
rootのcrontab（Born2BeRootはここが多い）
sudo crontab -l

monitoring.sh を手動実行（最強の証明）
bash /path/to/monitoring.sh

wall が動くか
wall "test"

✅ “スクリプトを編集せず止める方法は？”（超よく聞かれる）
...
# LVMとは何ですか？PV/VG/LVを説明してください。

LVMは、ディスクの容量を柔軟に管理する仕組みで、後からサイズ変更や拡張がしやすいのが特徴です。
PVはLVM用に初期化したディスクやパーティションで、これを材料にします。
VGはPVをまとめて作る“容量のプール”です。
LVはVGの中から切り出して作る領域で、見た目はパーティションのように使えて、必要に応じてサイズを変更できます。

今回は sda5 を PV（材料）にして、それを VG（容量プール）にまとめ、
その中から LV（論理ボリューム）として /（ルート）, /home, swap を作成している。
...
VG  : vgname（容量プール）
  ↓
LV  : lv_root   → /     （ext4などでフォーマットしてマウント）
LV  : lv_home   → /home （同上）
LV  : lv_swap   → swap  （swapとして有効化）


追加の追撃質問
LVMのメリットは？

サイズ変更しやすい、複数ディスクをまとめられる、スナップショットが取れるなど運用が楽になります。

✅ 証拠コマンド（本番で見せるやつ）

評価者に「じゃあ見せて」と言われたらこれでOKです。
...
lsblk -f：暗号化→LVMの構造がツリーで見える（超強い）
pvs/vgs/lvs：PV/VG/LVの実体を証明できる
df -hT：どのLVが /home /var などにマウントされてるか見せられる

# 暗号化（LUKS）とは何ですか？なぜ必要ですか？


暗号化は、ディスクの中身を鍵（パスフレーズ）がないと読めない状態にして、第三者に内容を見られないようにする仕組みです。
目的は、PCやディスクが盗難・紛失したり、ディスクを抜き取られてもデータを保護することです。
Born2BeRootでは LUKS で暗号化した領域の中にLVMを作って、起動時にパスフレーズを入力しないと中身にアクセスできないようにしています。

“今ちゃんと開いて使われてるか”の証拠
（名前は環境により sda5_crypt みたいに違う）
...
評価者が「じゃあ暗号化どこ？」って言ったらこれ一発です。
1発で構造を見せる（最強）
lsblk -f
✅ crypto_LUKS が見えたら 暗号化されてる証拠
...
# パーティションとは何ですか？なぜ分ける必要がありますか？

パーティションは、1つの物理ディスクを複数の区画に分割して、それぞれを別の領域として扱えるようにする仕組みです。
分ける理由は、用途ごとに領域を分離して管理しやすくし、障害や容量不足の影響を局所化するためです。
例えば /home や /var を分けることで、ログなどが増えてもシステム全体が壊れにくくなります。
Born2BeRootでは、この分割した領域の上に暗号化やLVMを組み合わせて、より安全で柔軟な構成にしています。

今回は/boot は起動の都合で分けて、残りを暗号化+LVMにする構成が一般的です。
